...
public:
        ...
        FileOperationType action)
    {
        ...
        if (FAILED(hr))
        {
            ...
            {
            case FileOperationType::COPY:
                pytrace(AMERROR, "CopyOperationFailed", fmt::format("[{}...]->{}", srcs[0], dst), "PerformOperations", GetErrorMsg(hr));
                break;
            case FileOperationType::MOVE:
                pytrace(AMERROR, "MoveOperationFailed", fmt::format("[{}...]->{}", srcs[0], dst), "PerformOperations", GetErrorMsg(hr));
                break;
            }
        ...
        if (aborted)
        {
            pytrace(AMINFO, "", fmt::format("[{}...]->{}", srcs[0], dst), "PerformOperations", "Operation aborted");
    ...
    TOR remove(std::vector<std::wstring> paths)
    {
        ...
        if (aborted)
        {
            pytrace(AMINFO, "", fmt::format("[{}...]->null", paths[0]), "remove", "Operation aborted");
            return FOR::OperationAborted;
        }
        return result;
    }
