...
public:
    ...
    FileOperationResult copy(std::vector<OT> tasks)
    {
        for (auto task : tasks)
        {
            IShellItem *pSourceItem = nullptr;
            HRESULT hr = SHCreateItemFromParsingName(task.src.c_str(), nullptr, IID_PPV_ARGS(&pSourceItem));
            if (FAILED(hr))
            {
            ...
            // 添加复制操作
            hr = pFileOp->CopyItem(pSourceItem, pDestinationItem, task.dst.c_str(), nullptr);
            if (FAILED(hr))
            {
                pSourceItem->Release();
