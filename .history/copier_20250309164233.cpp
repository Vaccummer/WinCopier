...
public:
    ...
    OR src2dst(std::vector<std::wstring> srcs, std::wstring dst)
    {
    }
    ...
    OR copy(std::vector<std::wstring> srcs, std::wstring dst)
    {
        ...
        for (auto src : srcs)
        {
            if (!fs::exists(src))
            {
                result.push_back(std::pair(src, FR::PathNotExists));
                continue;
            }
            ...
            if (FAILED(hr))
            {
                result.push_back(std::pair(src, FR::FailToCreSrcShellItem));
                continue;
            }
            ...
            {
                pSourceItem->Release();
                result.push_back(std::pair(src, FR::FailToAddOperation));
                continue;
            }
            result.push_back(std::pair(src, FR::SUCCESS));
        }
    ...
    OR action(const std::wstring &src, const std::wstring &dst, FileOperationType action)
    {
        if (pFileOp == nullptr)
        {
            return FileOperationResult::FailToCreateIFileOperationInstance;
