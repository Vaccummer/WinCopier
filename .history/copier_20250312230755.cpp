...
public:
    ...
    std::string GetErrorMsg(HRESULT hr)
    {
        ...
        std::string msg = lpwstr2str(errorMessage);
    ...
    FOR Config(FileOperationSet set)
    {
        ...
        if (FAILED(hr))
        {
            pytrace(AMCRITICAL, "ConfigFailed", "ExplorerAPI", "SetOperationFlags", "Failed to set operation flags");
    ...
    FOR Init(FileOperationSet set, py::object pycb = py::none())
    {
        ...
        if (FAILED(hr))
        {
            pytrace(AMCRITICAL, "InitFailed", "COMToolkit", "CoInitializeEx", "Failed to initialize COM");
        ...
        if (FAILED(hr))
        {
            pytrace(AMCRITICAL, "InstanceCreateFailed", "COMInstance", "CoCreateInstance", "Failed to create IFileOperation instance");
        ...
        FileOperationType action)
    {
        ...
        if (FAILED(hr))
        {
            pytrace(AMWARNING, "DstShellItemCreateFailed", tostr(dst), "SHCreateItemFromParsingName", "Failed to create destination shell item");
        ...
        for (const auto &src : srcs)
        {
            ...
            if (FAILED(hr))
            {
                pytrace(AMWARNING, "SrcShellItemCreateFailed", tostr(src), "SHCreateItemFromParsingName", "Failed to create source shell item");
